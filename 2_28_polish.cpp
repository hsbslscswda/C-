...
const int FACE_TEX_SIZE = 256;
FaceTexture gFaceTex[2]{};
bool gInspectForLeft = true;

// 本局摘要统计
DWORD gBattleStartTick = 0;
int gLeftHealthPackTaken = 0;
int gRightHealthPackTaken = 0;
int gLeftSkillPackTaken = 0;
int gRightSkillPackTaken = 0;
int gLeftYolkFired = 0;
int gRightYolkFired = 0;
wstring gLastDamageSource = L"尚未记录";

void resetBattleStats() {
    gBattleStartTick = GetTickCount();
    gLeftHealthPackTaken = 0;
    gRightHealthPackTaken = 0;
    gLeftSkillPackTaken = 0;
    gRightSkillPackTaken = 0;
    gLeftYolkFired = 0;
    gRightYolkFired = 0;
    gLastDamageSource = L"尚未记录";
}

int getBattleSeconds() {
    if (gBattleStartTick == 0) return 0;
    return (int)((GetTickCount() - gBattleStartTick) / 1000);
}
...
float clamp01(float x) {
    return clampFloat(x, 0.0f, 1.0f);
}

COLORREF blendColor(COLORREF base, COLORREF over, float alpha) {
    alpha = clamp01(alpha);
    int r = (int)(GetRValue(base) * (1.0f - alpha) + GetRValue(over) * alpha);
    int g = (int)(GetGValue(base) * (1.0f - alpha) + GetGValue(over) * alpha);
    int b = (int)(GetBValue(base) * (1.0f - alpha) + GetBValue(over) * alpha);
    return RGB(r, g, b);
}
...
float getAIStrengthFactor() {
    if (gArenaMode == ARENA_NIGHTMARE) return 3.15f;
    if (gArenaMode == ARENA_ADVANCED) return 1.85f;
    return 0.0f;
}
...
void getEggBodyProfile(const EggEntity& e, float& a, float& b, float& asym) {
...
float getEggHalfHeightAtNormX(const EggEntity& e, float u, float scale) {
    float a, b, asym;
    getEggBodyProfile(e, a, b, asym);
    u = clampFloat(u, -1.0f, 1.0f);
    ...
    return b * core * sideScale * belly * scale;
}

bool chooseFaceImageForSide(int side) {
    ...
    ofn.lpstrFilter = L"图片文件 (*.bmp;*.jpg;*.jpeg;*.png) *.bmp;*.jpg;*.jpeg;*.png 所有文件 (*.*) *.* ";
    ofn.nFilterIndex = 1;
    ofn.Flags = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST | OFN_HIDEREADONLY;
    if (!GetOpenFileNameW(&ofn)) return false;

    loadimage(&gFaceTex[side].img, fileName, FACE_TEX_SIZE, FACE_TEX_SIZE, true);
    gFaceTex[side].loaded = true;
    return true;
}

void clearFaceImageForSide(int side) {
    gFaceTex[side].loaded = false;
}

const wchar_t* getFaceOwnerText(bool forLeft) {
    return forLeft ? L"当前头像：左侧鸡蛋" : L"当前头像：右侧鸡蛋";
}

void drawEggFaceOverlayBySide(const EggEntity& e, float scale, int side) {
    if (side < 0 || side > 1) return;
    if (!gFaceTex[side].loaded) return;

    DWORD* buf = GetImageBuffer(&gFaceTex[side].img);
    if (!buf) return;

    int halfH = (int)(46.0f * scale);
    if (halfH < 12) return;

    for (int ly = -halfH; ly <= halfH; ++ly) {
        float ny = (float)ly / (float)halfH;
        float verticalBand = 1.0f - 0.28f * ny * ny - 0.14f * ny;
        float faceHalfW = (24.0f + 11.0f * verticalBand) * scale;
        if (faceHalfW < 8.0f) continue;

        for (int lx = (int)(-faceHalfW); lx <= (int)(faceHalfW); ++lx) {
            float nx = (float)lx / faceHalfW;
            float eggU = clampFloat(nx * (0.76f + 0.12f * (1.0f - fabsf(ny))), -0.92f, 0.92f);
            float shellHalfH = getEggHalfHeightAtNormX(e, eggU, scale);
            if (fabsf((float)ly) > shellHalfH - 2.0f) continue;

            float mask = nx * nx + ny * ny * 0.72f;
            if (mask > 1.0f) continue;

            float projX = clampFloat(nx * (0.86f - 0.10f * ny * ny), -0.995f, 0.995f);
            float u = asinf(projX) / PI_F + 0.5f;
            float v = clampFloat(powf(ny * 0.5f + 0.5f, 0.94f), 0.0f, 1.0f);
            int sx = (int)(u * (FACE_TEX_SIZE - 1));
            int sy = (int)(v * (FACE_TEX_SIZE - 1));
            if (sx < 0) sx = 0;
            if (sx >= FACE_TEX_SIZE) sx = FACE_TEX_SIZE - 1;
            if (sy < 0) sy = 0;
            if (sy >= FACE_TEX_SIZE) sy = FACE_TEX_SIZE - 1;

            DWORD raw = buf[sy * FACE_TEX_SIZE + sx];
            COLORREF src = (COLORREF)(raw & 0x00FFFFFF);
            unsigned int alphaByte = (raw >> 24) & 0xFF;
            float alpha = (alphaByte == 0 && (raw & 0x00FFFFFF) != 0) ? 0.90f : alphaByte / 255.0f;
            float edge = 1.0f - clamp01(nx * nx * 0.88f + ny * ny * 0.62f);
            alpha *= 0.26f + 0.74f * powf(edge, 0.55f);
            if (alpha < 0.02f) continue;

            Vec2 w = addVec(e.pos, rotateLocal(makeVec((float)lx, (float)ly), e.angle));
            int px = (int)w.x;
            int py = (int)w.y;
            if (px < 0 || px >= SCREEN_W || py < 0 || py >= SCREEN_H) continue;

            COLORREF dst = getpixel(px, py);
            COLORREF mixed = blendColor(dst, src, alpha);
            mixed = blendColor(mixed, RGB(255, 245, 236), 0.08f * edge);
            putpixel(px, py, mixed);
        }
    }

    setlinecolor(RGB(108, 86, 68));
    for (int ly = -halfH; ly <= halfH; ly += 3) {
        float ny = (float)ly / (float)halfH;
        float verticalBand = 1.0f - 0.28f * ny * ny - 0.14f * ny;
        float faceHalfW = (24.0f + 11.0f * verticalBand) * scale;
        float rimX = faceHalfW * (0.90f - 0.08f * fabsf(ny));
        Vec2 a = addVec(e.pos, rotateLocal(makeVec(-rimX, (float)ly), e.angle));
        Vec2 b = addVec(e.pos, rotateLocal(makeVec(rimX, (float)ly), e.angle));
        putpixel((int)a.x, (int)a.y, RGB(108, 86, 68));
        putpixel((int)b.x, (int)b.y, RGB(108, 86, 68));
    }
}
...
void resetAIState() {
    ...
    if (gArenaMode == ARENA_NIGHTMARE) gAINextAttackFrames = randInt(64, 112);
    else if (gArenaMode == ARENA_ADVANCED) gAINextAttackFrames = randInt(140, 220);
...
void drawBattleEggPanel(int x, int y, const wchar_t* title, const EggEntity& egg, float scaleMul) {
    ...
    drawEggFaceOverlayBySide(preview, 0.96f * scaleMul, x < SCREEN_W / 2 ? 0 : 1);
    ...
    outtextxy(x + 16, y + 212, s1.c_str());
    outtextxy(x + 16, y + 242, s2.c_str());
    outtextxy(x + 16, y + 272, s3.c_str());
    outtextxy(x + 16, y + 302, fitTextToWidth(getEggRevealLine1(egg.draft), 238).c_str());
}
...
void drawBattleLogBox(int x, int y, int w, int h, const wstring& line1, const wstring& line2, const wstring& line3) {
    ...
    outtextxy(x + 18, y + 8, L"对战日志");

    setUIFontAndColor(24, BLACK);
    outtextxy(x + 18, y + 48, fitTextToWidth(line1, w - 36).c_str());
    outtextxy(x + 18, y + 82, fitTextToWidth(line2, w - 36).c_str());
    outtextxy(x + 18, y + 116, fitTextToWidth(line3, w - 36).c_str());
}
...
bool tryPickupHealthPack(EggEntity& egg, const wchar_t* pickerName, wstring& log1, wstring& log2, wstring& log3) {
    for (int i = 0; i < MAX_HEALTH_PACKS; ++i) {
        ...
        if (pickerName && pickerName[0] == L'左') gLeftHealthPackTaken++;
        else gRightHealthPackTaken++;
...
bool tryPickupSkillPack(EggEntity& egg, int sideOwner, wstring& log1, wstring& log2, wstring& log3) {
    ...
    if (sideOwner == 0) { gLeftHasSkillShot = true; gLeftSkillPackTaken++; }
    else { gRightHasSkillShot = true; gRightSkillPackTaken++; }
...
void fireYolkShot(int owner, const EggEntity& shooter, wstring& log1, wstring& log2, wstring& log3) {
    ...
    if (owner == 0) { gLeftHasSkillShot = false; gLeftYolkFired++; }
    else { gRightHasSkillShot = false; gRightYolkFired++; }
...
void drawMenuScene(const Button& btnPVP, const Button& btnPVE, const Button& btnHelp, const Button& btnExit) {
    drawSceneBackground(RGB(196, 208, 222), RGB(124, 146, 170), RGB(168, 150, 122), RGB(62, 79, 99));

    int panelL = SCREEN_W / 2 - 330;
    int panelR = SCREEN_W / 2 + 330;
    int panelT = SCREEN_H / 2 - 250;
    int panelB = SCREEN_H / 2 + 260;

    drawCenteredTextAtY(SCREEN_W / 2, 46, L"蛋尖对决", 66, BLACK);
    drawSectionCaption(SCREEN_W / 2, 118, 220, L"实时竞技 · 资源争夺 · 战术 AI", 25, RGB(70, 76, 86), RGB(122, 106, 82));

    drawPanel(panelL, panelT, panelR, panelB, RGB(246, 244, 239), RGB(118, 128, 140), RGB(165, 150, 120));
    drawCenteredTextAtY(SCREEN_W / 2, panelT + 26, L"开始游戏", 36, RGB(58, 62, 66));
    drawCenteredTextAtY(SCREEN_W / 2, panelT + 74, L"先选对战模式，再选择地图难度与鸡蛋", 24, RGB(88, 88, 88));

    drawButton(btnPVP, true);
    drawButton(btnPVE, true);
    drawButton(btnHelp, true);
    drawButton(btnExit, true);
}

void drawHelpScene(const Button& btnBack) {
    drawSceneBackground(RGB(198, 210, 224), RGB(129, 150, 173), RGB(173, 154, 126), RGB(82, 72, 54));

    int panelL = SCREEN_W / 2 - 560;
    int panelR = SCREEN_W / 2 + 560;
    int panelT = 96;
    int panelB = SCREEN_H - 105;
    drawPanel(panelL, panelT, panelR, panelB, RGB(247, 245, 240), RGB(132, 122, 104), RGB(170, 152, 120));

    drawCenteredTextAtY(SCREEN_W / 2, 38, L"玩法说明", 52, BLACK);
    drawCenteredTextAtY(SCREEN_W / 2, 116, L"用走位、角度、资源和环境机制打败对手", 28, RGB(62, 56, 48));

    setUIFontAndColor(25, BLACK);
    int x1 = panelL + 60;
    int x2 = panelL + 580;
    int y = 168;
    outtextxy(x1, y, L"基础操作");
    outtextxy(x2, y, L"地图机制");

    setUIFontAndColor(23, RGB(48, 48, 48));
    y += 46;
    outtextxy(x1, y, L"P1：WASD 移动，Q/E 微调，G 加速，F 发射蛋黄");
    outtextxy(x2, y, L"基础：2 个障碍，血包常驻刷新");
    y += 38;
    outtextxy(x1, y, L"P2：方向键移动，K/L 微调，右 Shift 加速，右 Ctrl 发射");
    outtextxy(x2, y, L"进阶：4 个障碍，1 个大型尖刺，血包跟随尖刺预警");
    y += 38;
    outtextxy(x1, y, L"血包：恢复耐久；技能包：获得一次蛋黄远程攻击");
    outtextxy(x2, y, L"噩梦：4 个障碍，2 个巨型尖刺，强压迫 AI");

    y += 68;
    setUIFontAndColor(25, BLACK);
    outtextxy(x1, y, L"战斗技巧");
    outtextxy(x2, y, L"AI 难度");

    setUIFontAndColor(23, RGB(48, 48, 48));
    y += 46;
    outtextxy(x1, y, L"尖端适合主动攻击，钝端更适合承压，侧面要谨慎暴露");
    outtextxy(x2, y, L"基础 AI：适合熟悉操作，会进攻但判断保守");
    y += 38;
    outtextxy(x1, y, L"加速撞击能打出更强击退，也会消耗体力");
    outtextxy(x2, y, L"进阶 AI：会抢资源、拉扯、远程消耗和追击弱点");
    y += 38;
    outtextxy(x1, y, L"尖刺预警轨道既危险，也可能伴随短时血包机会");
    outtextxy(x2, y, L"噩梦 AI：会控场、骗冲刺、保护残血端并压迫走位");

    y += 68;
    drawSectionCaption(SCREEN_W / 2, y, 320, L"头像模式", 25, RGB(50, 50, 50), RGB(122, 106, 82));
    setUIFontAndColor(23, RGB(48, 48, 48));
    drawCenteredTextAtY(SCREEN_W / 2, y + 48, L"进入放大检视页后可上传头像；建议使用正脸照片，先预览再开战。", 23, RGB(48, 48, 48));
...
void drawMapSelectScene(const Button& btnBasic, const Button& btnAdvanced, const Button& btnNightmare, const Button& btnBack, int currentMode) {
    ...
    setUIFontAndColor(21, RGB(60, 60, 60));
    outtextxy(btnBasic.left + 22, btnBasic.bottom + 8, L"适合熟悉操作：2 个障碍，资源稳定，AI 均衡");
    outtextxy(btnAdvanced.left + 22, btnAdvanced.bottom + 8, L"战术压力提升：4 个障碍，大型尖刺，AI 会主动抢资源");
    outtextxy(btnNightmare.left + 22, btnNightmare.bottom + 8, L"高压挑战：巨型尖刺、高频压迫、噩梦 AI 控场追击");
...
void drawInspectScene(
    ...
    const Button& btnUploadFace,
    const Button& btnClearFace,
    ...
    drawEggFaceOverlayBySide(preview, 1.98f, gInspectForLeft ? 0 : 1);
    ...
    setUIFontAndColor(22, RGB(66, 60, 52));
    outtextxy(rightL + 28, 566, getFaceOwnerText(gInspectForLeft));
    outtextxy(rightL + 28, 600, gFaceTex[gInspectForLeft ? 0 : 1].loaded ? L"状态：已加载" : L"状态：未加载");

    drawButton(btnUploadFace, true);
    drawButton(btnClearFace, true);
...
void drawBattleScene(
    ...
    drawCenteredTextAtY(SCREEN_W / 2, 10, L"对战中", 40, BLACK);
    drawCenteredTextAtY(SCREEN_W / 2, 54, getArenaModeText(gArenaMode), 26, RGB(70, 70, 70));
    if (currentMode == 1) {
        setUIFontAndColor(24, BLACK);
        outtextxy(84, 20, L"P1: WASD 移动，Q/E 微调，G 加速，F 发射蛋黄");
        outtextxy(SCREEN_W - 620, 20, L"P2: 方向键移动，K/L 微调，RShift 加速，右Ctrl 发射蛋黄");
    }
    else {
        setUIFontAndColor(24, BLACK);
        outtextxy(66, 20, L"你: WASD 移动，Q/E 微调，G 加速，F 发射蛋黄，按住鼠标左键可辅助转向");
        outtextxy(SCREEN_W - 150, 20, L"AI");
    }

    if (openingFrames > 0) {
        setUIFontAndColor(28, RGB(80, 40, 20));
        outtextxy(SCREEN_W / 2 - 180, 86, L"开局试探期：暂时不能加速");
    }
    ...
    setUIFontAndColor(24, RGB(70, 56, 18));
    outtextxy(110, 116, gLeftHasSkillShot ? L"左侧技能：蛋黄已就绪" : L"左侧技能：未持有");
    outtextxy(SCREEN_W - 410, 116, gRightHasSkillShot ? L"右侧技能：蛋黄已就绪" : L"右侧技能：未持有");
    ...
    drawEggShadow(l, ls);
    drawEggShape(l, ls);
    drawEggFaceOverlayBySide(l, ls, 0);
    drawEggCracks(l, ls);

    drawEggShadow(r, rs);
    drawEggShape(r, rs);
    drawEggFaceOverlayBySide(r, rs, 1);
    drawEggCracks(r, rs);

    drawBattleLogBox(SCREEN_W / 2 - 430, SCREEN_H - 198, 860, 156, log1, log2, log3);
    drawButton(btnBack, true);
}
...
void drawResultScene(
    ...
    int summaryL = SCREEN_W / 2 - 390;
    int summaryT = 530;
    drawPanel(summaryL, summaryT, summaryL + 780, summaryT + 154, RGB(247, 245, 240), RGB(132, 122, 104), RGB(170, 152, 120));
    drawCenteredTextAtY(SCREEN_W / 2, summaryT + 10, L"本局摘要", 27, BLACK);

    int battleSec = getBattleSeconds();
    wstring timeText = L"对战时长：" + toW(battleSec / 60) + L"分" + toW(battleSec % 60) + L"秒";
    wstring leftText = L"左侧：剩余完整度 " + toW(leftEgg.totalIntegrity) + L"/" + toW(leftEgg.totalMaxIntegrity) +
        L"，血包 " + toW(gLeftHealthPackTaken) + L"，技能包 " + toW(gLeftSkillPackTaken) + L"，蛋黄 " + toW(gLeftYolkFired);
    wstring rightText = L"右侧：剩余完整度 " + toW(rightEgg.totalIntegrity) + L"/" + toW(rightEgg.totalMaxIntegrity) +
        L"，血包 " + toW(gRightHealthPackTaken) + L"，技能包 " + toW(gRightSkillPackTaken) + L"，蛋黄 " + toW(gRightYolkFired);
    wstring finalText = L"最后关键来源：" + gLastDamageSource;

    setUIFontAndColor(21, BLACK);
    outtextxy(summaryL + 30, summaryT + 48, timeText.c_str());
    outtextxy(summaryL + 30, summaryT + 78, fitTextToWidth(leftText, 720).c_str());
    outtextxy(summaryL + 30, summaryT + 108, fitTextToWidth(rightText, 720).c_str());
    outtextxy(summaryL + 30, summaryT + 138, fitTextToWidth(finalText, 720).c_str());
...
Vec2 computeAIDangerVector(const EggEntity& ai, const EggEntity& target) {
    ...
    for (int i = 0; i < 2; ++i) {
        if (!gYolkShots[i].active) continue;
        if (gYolkShots[i].owner != 0) continue;
        Vec2 rel = subVec(ai.pos, gYolkShots[i].pos);
        float dist = lenVec(rel);
        if (dist > 240.0f || dist < 0.01f) continue;
        Vec2 shotDir = normVec(gYolkShots[i].vel);
        Vec2 relDir = normVec(rel);
        float closing = dotVec(shotDir, relDir);
        if (closing > 0.55f) {
            Vec2 lateral = makeVec(-shotDir.y, shotDir.x);
            float sign = dotVec(lateral, relDir) >= 0.0f ? 1.0f : -1.0f;
            float k = (240.0f - dist) / 240.0f;
            avoid = addVec(avoid, mulVec(lateral, sign * (1.05f + 0.85f * k)));
            avoid = addVec(avoid, mulVec(relDir, 0.40f * k));
        }
    }

    Vec2 toAI = subVec(ai.pos, target.pos);
    float playerDist = lenVec(toAI);
    if (playerDist > 0.01f && playerDist < 240.0f) {
        Vec2 playerDir = eggForward(target);
        Vec2 towardAI = normVec(toAI);
        float lining = dotVec(playerDir, towardAI);
        if ((target.boosting || lenVec(target.vel) > 5.2f) && lining > 0.60f) {
            Vec2 lateral = makeVec(-towardAI.y, towardAI.x);
            float sign = dotVec(lateral, eggForward(ai)) >= 0.0f ? 1.0f : -1.0f;
            float k = (240.0f - playerDist) / 240.0f;
            avoid = addVec(avoid, mulVec(lateral, sign * (0.90f + 0.55f * k)));
        }
    }
...
float getEggIntegrityRatio(const EggEntity& e) {
    if (e.totalMaxIntegrity <= 0) return 0.0f;
    return (float)e.totalIntegrity / (float)e.totalMaxIntegrity;
}

bool findBestHealthPackTarget(const EggEntity& ai, const EggEntity& target, Vec2& outTarget) {
    float bestScore = 1e9f;
    bool found = false;
    float hpRatio = getEggIntegrityRatio(ai);

    for (int i = 0; i < MAX_HEALTH_PACKS; ++i) {
        if (!gHealthPacks[i].active) continue;
        float aiDist = lenVec(subVec(gHealthPacks[i].pos, ai.pos));
        float targetDist = lenVec(subVec(gHealthPacks[i].pos, target.pos));
        float urgency = (1.0f - hpRatio) * 300.0f;
        float contest = clampFloat(targetDist - aiDist, -180.0f, 180.0f) * 0.60f;
        float score = aiDist - urgency - contest;
        bool shouldGo = hpRatio < 0.82f || aiDist + 26.0f < targetDist || (isNightmareMode() && hpRatio < 0.94f && aiDist < targetDist + 70.0f);
        if (shouldGo && score < bestScore) {
            bestScore = score;
            outTarget = gHealthPacks[i].pos;
            found = true;
        }
    }
    return found;
...
bool findBestSkillPackTarget(const EggEntity& ai, const EggEntity& target, Vec2& outTarget) {
    if (!gSkillPack.active || gRightHasSkillShot) return false;

    float aiDist = lenVec(subVec(gSkillPack.pos, ai.pos));
    float targetDist = lenVec(subVec(gSkillPack.pos, target.pos));
    bool shouldGo = aiDist + 16.0f < targetDist || isNightmareMode() || aiDist < 220.0f;
    if (!shouldGo) return false;

    outTarget = gSkillPack.pos;
    return true;
}

float getEndHpRatio(const EggEntity& e, ContactRegion r) {
    if (r == REGION_SHARP) return e.sharpMaxHP <= 0 ? 0.0f : (float)e.sharpHP / (float)e.sharpMaxHP;
    if (r == REGION_BLUNT) return e.bluntMaxHP <= 0 ? 0.0f : (float)e.bluntHP / (float)e.bluntMaxHP;
    return e.sideMaxHP <= 0 ? 0.0f : (float)e.sideHP / (float)e.sideMaxHP;
}

float getLowestEndHpRatio(const EggEntity& e) {
    float a = getEndHpRatio(e, REGION_SHARP);
    float b = getEndHpRatio(e, REGION_BLUNT);
    float c = getEndHpRatio(e, REGION_SIDE);
    float m = a < b ? a : b;
    return m < c ? m : c;
}

int getBestAttackValue(const EggEntity& e) {
    return e.draft.sharpAttack > e.draft.bluntAttack ? e.draft.sharpAttack : e.draft.bluntAttack;
}

int getBestDefenseValue(const EggEntity& e) {
    int m = e.draft.sharpDefense > e.draft.bluntDefense ? e.draft.sharpDefense : e.draft.bluntDefense;
    int side = (e.draft.sharpDefense + e.draft.bluntDefense) / 2;
    return m > side ? m : side;
}

float getCombatPowerScore(const EggEntity& e) {
    return (float)getBestAttackValue(e) * 1.25f + (float)getBestDefenseValue(e) * 0.75f + getEggIntegrityRatio(e) * 5.0f + e.stamina * 0.015f;
}

Vec2 getClosestArenaSafePoint(Vec2 from, Vec2 desired) {
    Vec2 p = desired;
    float margin = 90.0f;
    if (p.x < ARENA_L + margin) p.x = ARENA_L + margin;
    if (p.x > ARENA_R - margin) p.x = ARENA_R - margin;
    if (p.y < ARENA_T + margin) p.y = ARENA_T + margin;
    if (p.y > ARENA_B - margin) p.y = ARENA_B - margin;

    for (int i = 0; i < gObstacleCount; ++i) {
        if (!gObstacles[i].active) continue;
        Vec2 away = subVec(p, gObstacles[i].pos);
        float d = lenVec(away);
        float safe = gObstacles[i].radius + 105.0f;
        if (d < safe && d > 0.01f) {
            p = addVec(gObstacles[i].pos, mulVec(normVec(away), safe));
        }
    }
    return p;
}

Vec2 getArenaCenterPoint() {
    return makeVec((ARENA_L + ARENA_R) * 0.5f, (ARENA_T + ARENA_B) * 0.5f);
}

float getResourceNeedScore(const EggEntity& ai, const EggEntity& target) {
    float selfHp = getEggIntegrityRatio(ai);
    float targetHp = getEggIntegrityRatio(target);
    float selfWeak = getLowestEndHpRatio(ai);
    float targetWeak = getLowestEndHpRatio(target);
    float powerGap = getCombatPowerScore(target) - getCombatPowerScore(ai);
    float need = 0.0f;

    need += (1.0f - selfHp) * 80.0f;
    need += (1.0f - selfWeak) * 40.0f;
    if (selfHp < targetHp) need += 28.0f;
    if (selfHp + 0.10f < targetHp) need += 30.0f;
    if (powerGap > 0.0f) need += powerGap * 7.5f;
    if (targetWeak < 0.25f && selfHp > 0.45f) need -= 22.0f;
    return need;
}

Vec2 predictTargetPositionForAI(const EggEntity& target, float level, float frames) {
    float lead = frames + 3.0f * level;
    Vec2 p = addVec(target.pos, mulVec(target.vel, lead));
    float margin = 80.0f;
    if (p.x < ARENA_L + margin) p.x = ARENA_L + margin;
    if (p.x > ARENA_R - margin) p.x = ARENA_R - margin;
    if (p.y < ARENA_T + margin) p.y = ARENA_T + margin;
...
Vec2 computeInterceptPointNearResource(const EggEntity& ai, const EggEntity& target, Vec2 resPos) {
    Vec2 fromPlayer = subVec(resPos, target.pos);
    float d = lenVec(fromPlayer);
    if (d < 0.01f) return resPos;
    Vec2 dirPlayer = normVec(fromPlayer);
    Vec2 block = subVec(resPos, mulVec(dirPlayer, 92.0f));
    return getClosestArenaSafePoint(ai.pos, block);
}
...
void updateAIControl(EggEntity& ai, const EggEntity& target, bool allowBoost) {
    ...
    float selfWeak = getLowestEndHpRatio(ai);
    float targetWeak = getLowestEndHpRatio(target);
    float selfPower = getCombatPowerScore(ai);
    float targetPower = getCombatPowerScore(target);
    float powerGap = selfPower - targetPower;
    bool isAdvanced = gArenaMode == ARENA_ADVANCED;
    bool isNight = gArenaMode == ARENA_NIGHTMARE;
    ...
    if (ai.hitFlashFrames >= (level >= 2.0f ? 4 : 6) && gAIState != AI_RECOVER) {
        startAIRecover();
    }

    Vec2 healthTarget = makeVec(0.0f, 0.0f);
    Vec2 skillTarget = makeVec(0.0f, 0.0f);
    bool wantsHealthPack = findBestHealthPackTarget(ai, target, healthTarget);
    bool wantsSkillPack = findBestSkillPackTarget(ai, target, skillTarget);

    Vec2 dangerVec = computeAIDangerVector(ai, target);
    bool hasDanger = lenVec(dangerVec) > (isNight ? 0.055f : (isAdvanced ? 0.075f : 0.12f));

    float resourceNeed = getResourceNeedScore(ai, target);
    bool losingFight = (selfHp + 0.05f < targetHp) || (selfHp <= targetHp + 0.03f && powerGap < -0.45f) || selfWeak < 0.22f;
    bool strongAdvantage = (selfHp > targetHp + 0.14f) || powerGap > 1.6f || targetWeak < 0.30f;
    bool finishTarget = targetHp < (isNight ? 0.55f : (isAdvanced ? 0.45f : 0.32f)) || targetWeak < (isNight ? 0.36f : 0.24f);

    Vec2 predicted = predictTargetPositionForAI(target, level, isNight ? 20.0f : (isAdvanced ? 14.0f : 9.0f));

    float healthScore = -9999.0f;
    if (wantsHealthPack) {
        float aiDist = lenVec(subVec(healthTarget, ai.pos));
        float playerDist = lenVec(subVec(healthTarget, target.pos));
        healthScore = resourceNeed + (playerDist - aiDist) * (isNight ? 0.55f : 0.36f) - aiDist * 0.07f;
        if (selfHp < 0.45f) healthScore += 55.0f;
        if (selfWeak < 0.20f) healthScore += 50.0f;
        if (targetHp < 0.35f && selfHp > 0.62f) healthScore -= 22.0f;
    }

    float skillScore = -9999.0f;
    if (wantsSkillPack) {
        float aiDist = lenVec(subVec(skillTarget, ai.pos));
        float playerDist = lenVec(subVec(skillTarget, target.pos));
        skillScore = 48.0f + (playerDist - aiDist) * (isNight ? 0.50f : 0.32f) - aiDist * 0.05f;
        if (losingFight) skillScore += 42.0f;
        if (gRightHasSkillShot) skillScore = -9999.0f;
        if (isNight) skillScore += 24.0f;
    }

    float avoidScore = hasDanger ? 200.0f : -9999.0f;
    float finishScore = -9999.0f;
    if (finishTarget && dist < (isNight ? 780.0f : 560.0f)) {
        finishScore = 62.0f + (1.0f - targetHp) * 70.0f + (1.0f - targetWeak) * 55.0f + powerGap * 6.0f;
        if (selfHp < 0.28f) finishScore -= 45.0f;
        if (gRightHasSkillShot) finishScore += 22.0f;
    }

    float kiteScore = -9999.0f;
    if (losingFight || selfHp < 0.42f || selfWeak < 0.26f) {
        kiteScore = 70.0f + (1.0f - selfHp) * 60.0f + (targetPower - selfPower) * 8.0f;
        if (gRightHasSkillShot) kiteScore += 28.0f;
        if (wantsHealthPack) kiteScore -= 15.0f;
    }

    float attackScore = 28.0f + powerGap * 9.0f + (targetHp < selfHp ? 20.0f : 0.0f) + (1.0f - targetWeak) * 34.0f;
    if (losingFight) attackScore -= (isNight ? 22.0f : 45.0f);
    if (strongAdvantage) attackScore += isNight ? 40.0f : 24.0f;
    if (dist < 120.0f) attackScore -= 25.0f;
    if (dist > 650.0f) attackScore -= 18.0f;
    if (gRightHasSkillShot && losingFight) attackScore -= 20.0f;

    float interceptScore = -9999.0f;
    if (wantsHealthPack && targetHp < 0.55f && selfHp > 0.42f) {
        float aiDist = lenVec(subVec(healthTarget, ai.pos));
        float playerDist = lenVec(subVec(healthTarget, target.pos));
        if (aiDist < playerDist + (isNight ? 160.0f : 80.0f)) {
            interceptScore = 85.0f + (1.0f - targetHp) * 50.0f - fabsf(aiDist - playerDist) * 0.10f;
        }
    }

    if (gAIState == AI_AVOID) {
        gAIAvoidFrames--;
        if (gAIAvoidFrames <= 0 && !hasDanger) gAIState = AI_PATROL;
    }
    ...
    if (gAIState != AI_RECOVER) {
        if (avoidScore > 0.0f) {
            gAIState = AI_AVOID;
            gAIAvoidFrames = isNight ? 34 : (isAdvanced ? 28 : 18);
        } else {
            float best = attackScore;
            AIState bestState = AI_ATTACK;
            if (kiteScore > best) { best = kiteScore; bestState = AI_PATROL; }
            if (healthScore > best) { best = healthScore; bestState = AI_LOOT_HEALTH; }
            if (skillScore > best) { best = skillScore; bestState = AI_LOOT_SKILL; }
            if (interceptScore > best) { best = interceptScore; bestState = AI_FINISH; healthTarget = computeInterceptPointNearResource(ai, target, healthTarget); }
            if (finishScore > best) { best = finishScore; bestState = AI_FINISH; }

            if (gArenaMode == ARENA_BASIC) {
                if (healthScore > 82.0f) gAIState = AI_LOOT_HEALTH;
                else if (finishScore > 110.0f) gAIState = AI_FINISH;
                else if (!losingFight && gAIState == AI_ATTACK) gAIState = AI_ATTACK;
                else gAIState = AI_PATROL;
            } else {
                gAIState = bestState;
            }
        }
    ...
    int nextMin = 300, nextMax = 460;
    if (gArenaMode == ARENA_ADVANCED) { nextMin = 140; nextMax = 220; }
    if (gArenaMode == ARENA_NIGHTMARE) { nextMin = 64; nextMax = 112; }

    float attackStaminaNeed = 24.0f;
    if (isAdvanced) attackStaminaNeed = 13.0f;
    if (isNight) attackStaminaNeed = 7.0f;

    if ((gAIState == AI_PATROL || gAIState == AI_ATTACK) &&
        gAINextAttackFrames <= 0 &&
        allowBoost && ai.stamina > attackStaminaNeed &&
        dist > 120.0f &&
        dist < (isNight ? 820.0f : (isAdvanced ? 680.0f : 520.0f)) &&
        !hasDanger &&
        (!losingFight || isNight || gRightHasSkillShot)) {
        gAIState = AI_ATTACK;
        gAIAttackFrames = isNight ? randInt(140, 210) : (isAdvanced ? randInt(104, 158) : randInt(58, 92));
        gAINextAttackFrames = randInt(nextMin, nextMax);
    }

    if (gAIState == AI_ATTACK) {
        gAIAttackFrames--;
        if (gAIAttackFrames <= 0 || hasDanger || (losingFight && !isNight && !gRightHasSkillShot)) {
            gAIState = AI_PATROL;
        }
    }
    ...
    float turnSpeed = 0.050f + 0.010f * level;
    float baseSpeed = 2.78f + 0.12f * level;
    bool wantsBoost = false;

    Vec2 targetForward = eggForward(target);
    Vec2 targetFutureDir = normVec(subVec(predicted, ai.pos));
    Vec2 safeCenter = getArenaCenterPoint();
    Vec2 towardCenter = normVec(subVec(safeCenter, ai.pos));
    Vec2 targetToHealth = wantsHealthPack ? normVec(subVec(healthTarget, target.pos)) : makeVec(0.0f, 0.0f);

    if (gAIState == AI_AVOID) {
        Vec2 extra = addVec(mulVec(side, 0.55f * gAIOrbitBias), mulVec(towardCenter, 0.38f));
        moveDir = normVec(addVec(dangerVec, extra));
        desiredAngle = (float)atan2((double)moveDir.y, (double)moveDir.x);
        turnSpeed = 0.080f + 0.016f * level;
        baseSpeed = 3.30f + 0.30f * level;
        wantsBoost = allowBoost && ai.stamina > (isNight ? 6.0f : 14.0f) && lenVec(dangerVec) > 0.45f;
    } else if (gAIState == AI_RECOVER) {
        Vec2 retreat = normVec(addVec(mulVec(dir, -1.05f), mulVec(side, (1.18f + 0.10f * level) * gAIOrbitBias)));
        Vec2 safety = addVec(retreat, mulVec(towardCenter, 0.34f));
        moveDir = normVec(safety);
        desiredAngle = (float)atan2((double)moveDir.y, (double)moveDir.x);
        turnSpeed = 0.066f + 0.012f * level;
        baseSpeed = 3.10f + 0.18f * level;
        wantsBoost = allowBoost && ai.stamina > 18.0f && selfHp < 0.48f;
    } else if (gAIState == AI_LOOT_HEALTH || gAIState == AI_LOOT_SKILL) {
        Vec2 resPos = (gAIState == AI_LOOT_HEALTH) ? healthTarget : skillTarget;
        Vec2 toRes = subVec(resPos, ai.pos);
        float resDist = lenVec(toRes);
        Vec2 resDir = normVec(toRes);
        Vec2 resSide = makeVec(-resDir.y, resDir.x);
        float contestBias = dotVec(resSide, dir) * (isNight ? 0.42f : 0.26f) + 0.20f * gAIOrbitBias;
        if (gAIState == AI_LOOT_HEALTH && targetHp < 0.45f && selfHp > 0.48f) {
            Vec2 block = computeInterceptPointNearResource(ai, target, resPos);
            resDir = normVec(subVec(block, ai.pos));
            resSide = makeVec(-resDir.y, resDir.x);
        }
        moveDir = normVec(addVec(resDir, mulVec(resSide, contestBias)));
        desiredAngle = (float)atan2((double)moveDir.y, (double)moveDir.x);
        turnSpeed = 0.074f + 0.012f * level;
        baseSpeed = 3.18f + 0.20f * level;
        wantsBoost = allowBoost && ai.stamina > (isNight ? 7.0f : 13.0f) && resDist > 90.0f && resDist < 520.0f;
    } else if (gAIState == AI_FINISH) {
        Vec2 chase = subVec(predicted, ai.pos);
        Vec2 chaseDir = normVec(chase);
        Vec2 chaseSide = makeVec(-chaseDir.y, chaseDir.x);
        float weakPressure = dotVec(chaseSide, mulVec(targetForward, -1.0f));
        float attackBias = 0.10f * gAIOrbitBias + 0.34f * weakPressure;
        if (isNight && isSpikeMode()) attackBias += 0.12f * gAIOrbitBias;
        moveDir = normVec(addVec(chaseDir, mulVec(chaseSide, attackBias)));
        desiredAngle = (float)atan2((double)moveDir.y, (double)moveDir.x);
        turnSpeed = 0.078f + 0.015f * level;
        baseSpeed = 3.42f + 0.28f * level;
        wantsBoost = allowBoost && ai.stamina > (isNight ? 8.0f : 14.0f) && dist > 105.0f && dist < 540.0f;
    } else if (gAIState == AI_ATTACK) {
        Vec2 attackDir = targetFutureDir;
        if (lenVec(attackDir) < 0.01f) attackDir = dir;
        Vec2 attackSide = makeVec(-attackDir.y, attackDir.x);
        float baitWave = sinf(nowTimeSec() * (3.1f + 0.35f * level));
        float attackBias = 0.18f * gAIOrbitBias + 0.20f * baitWave;
        if (targetWeak < 0.38f) attackBias += 0.28f * dotVec(attackSide, mulVec(targetForward, -1.0f));
        if (losingFight && gRightHasSkillShot) attackBias += 0.42f * gAIOrbitBias;
        moveDir = normVec(addVec(attackDir, mulVec(attackSide, attackBias)));
        desiredAngle = (float)atan2((double)moveDir.y, (double)moveDir.x);
        turnSpeed = 0.072f + 0.014f * level;
        baseSpeed = 3.30f + 0.30f * level;
        float alignedLimit = isNight ? 0.34f : (isAdvanced ? 0.38f : 0.28f);
        bool aligned = fabsf(angleDiff(desiredAngle, ai.angle)) < alignedLimit;
        wantsBoost = allowBoost && aligned && ai.stamina > (isNight ? 7.0f : (isAdvanced ? 12.0f : 18.0f)) && dist > 110.0f && dist < (isNight ? 620.0f : 460.0f);
    } else {
        if (losingFight || selfWeak < 0.30f) {
            Vec2 flee = normVec(addVec(mulVec(dir, -0.70f), mulVec(side, (1.22f + 0.10f * level) * gAIOrbitBias)));
            Vec2 safe = addVec(flee, mulVec(towardCenter, 0.30f));
            if (gRightHasSkillShot) safe = addVec(safe, mulVec(dir, -0.35f));
            moveDir = normVec(safe);
        } else if (dist > 400.0f - 34.0f * level) {
            moveDir = normVec(addVec(mulVec(dir, 0.85f), mulVec(side, (0.48f + 0.06f * level) * gAIOrbitBias)));
        } else if (dist < 165.0f + 8.0f * level) {
            moveDir = normVec(addVec(mulVec(dir, -0.72f), mulVec(side, (1.04f + 0.10f * level) * gAIOrbitBias)));
        } else {
            Vec2 orbit = addVec(mulVec(dir, isNight ? 0.30f : 0.18f), mulVec(side, (1.16f - 0.06f * level) * gAIOrbitBias));
            if (wantsHealthPack && targetToHealth.x != 0.0f) orbit = addVec(orbit, mulVec(targetToHealth, 0.18f));
            moveDir = normVec(orbit);
        }
        desiredAngle = (float)atan2((double)moveDir.y, (double)moveDir.x);
        turnSpeed = 0.054f + 0.012f * level;
        baseSpeed = 2.88f + 0.18f * level;
        wantsBoost = allowBoost && isNight && ai.stamina > 28.0f && (losingFight || dist > 480.0f);
    }

    if (lenVec(moveDir) < 0.01f) moveDir = normVec(addVec(mulVec(dir, -0.35f), mulVec(side, gAIOrbitBias)));
    ...
    float finalSpeed = wantsBoost ? baseSpeed * (2.06f + 0.08f * level) : baseSpeed;
    if (losingFight && !wantsBoost && gArenaMode != ARENA_BASIC) finalSpeed += 0.18f * level;
    Vec2 targetVel = mulVec(moveDir, finalSpeed);
    float inertia = isNight ? 0.60f : (isAdvanced ? 0.64f : 0.68f);
    ai.vel = addVec(mulVec(ai.vel, inertia), mulVec(targetVel, 1.0f - inertia));

    if (wantsBoost) ai.stamina -= (0.62f + 0.04f * level);
    else ai.stamina += (dist < 150.0f ? 0.62f : 0.38f);
...
void updateBattle(
    ...
    if (currentMode == 1) {
        ...
        if (gRightHasSkillShot && gArenaMode != ARENA_BASIC) {
            ...
            if (align < (gArenaMode == ARENA_NIGHTMARE ? 0.34f : 0.28f) &&
                dist > 90.0f && dist < (gArenaMode == ARENA_NIGHTMARE ? 760.0f : 540.0f) &&
                (gAIState == AI_ATTACK || gAIState == AI_FINISH || gAIState == AI_PATROL || gAIState == AI_RECOVER)) {
                fireYolkShot(1, rightEgg, log1, log2, log3);
                playEffect(L"hit_light.wav");
            }
        }
...
int main() {
    ...
    Button btnPVE = { centerX - menuW / 2, menuBaseY + 92, centerX + menuW / 2, menuBaseY + 92 + menuH, L"人机对战" };
    Button btnHelp = { centerX - menuW / 2, menuBaseY + 184, centerX + menuW / 2, menuBaseY + 184 + menuH, L"玩法说明" };
    Button btnExit = { centerX - menuW / 2, menuBaseY + 276, centerX + menuW / 2, menuBaseY + 276 + menuH, L"退出游戏" };
    Button btnHelpBack = { centerX - 140, SCREEN_H - 92, centerX + 140, SCREEN_H - 32, L"返回主菜单" };
    ...
    Button btnInspectRotL = { SCREEN_W / 2 - 390, SCREEN_H - 96, SCREEN_W / 2 - 250, SCREEN_H - 30, L"左转" };
    Button btnInspectRotR = { SCREEN_W / 2 - 235, SCREEN_H - 96, SCREEN_W / 2 - 95, SCREEN_H - 30, L"右转" };
    Button btnInspectUpload = { SCREEN_W / 2 - 10, SCREEN_H - 96, SCREEN_W / 2 + 180, SCREEN_H - 30, L"上传头像" };
    Button btnInspectClear = { SCREEN_W / 2 + 195, SCREEN_H - 96, SCREEN_W / 2 + 355, SCREEN_H - 30, L"清除头像" };
    Button btnInspectPick = { SCREEN_W / 2 - 110, SCREEN_H - 166, SCREEN_W / 2 + 110, SCREEN_H - 106, L"选择这颗蛋" };
    Button btnInspectBack = { SCREEN_W / 2 + 370, SCREEN_H - 96, SCREEN_W / 2 + 530, SCREEN_H - 30, L"返回选蛋" };
    ...
    while (running) {
            ...
            if (msg.message == WM_LBUTTONDOWN) {
                ...
                if (currentScene == SCENE_MENU) {
                    ...
                    if (isInside(btnBackSelect, x, y)) {
                        ...
                        if (chosen != -1) {
                            ...
                            if (chosen != -1) {
                                ...
                                gInspectForLeft = !(currentMode == 1 && selectStep == 2);
                    ...
                    if (isInside(btnInspectRotL, x, y)) {
                        ...
                        playEffect(L"ui_click.wav");
                        inspectAngle += 0.18f;
                    } else if (isInside(btnInspectUpload, x, y)) {
                        playEffect(L"ui_click.wav");
                        chooseFaceImageForSide(gInspectForLeft ? 0 : 1);
                    } else if (isInside(btnInspectClear, x, y)) {
                        playEffect(L"ui_click.wav");
                        clearFaceImageForSide(gInspectForLeft ? 0 : 1);
        ...
        if (currentScene == SCENE_MENU) {
            drawMenuScene(btnPVP, btnPVE, btnHelp, btnExit);
        } else if (currentScene == SCENE_HELP) {
            drawHelpScene(btnHelpBack);
        } else if (currentScene == SCENE_MAP_SELECT) {
            ...
            drawInspectScene(btnInspectRotL, btnInspectRotR, btnInspectUpload, btnInspectClear, btnInspectPick, btnInspectBack, inspectEgg, inspectIndex, inspectAngle);
        } else if (currentScene == SCENE_BATTLE) {
