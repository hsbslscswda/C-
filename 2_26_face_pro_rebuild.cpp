...
void drawEggFaceOverlayBySide(const EggEntity& e, float scale, int side) {
    ...
    int halfW = (int)(26.0f * scale);
    int halfH = (int)(32.0f * scale);
    if (halfW < 8 || halfH < 8) return;

    for (int ly = -halfH; ly <= halfH; ++ly) {
        for (int lx = -halfW; lx <= halfW; ++lx) {
            float nx = (float)lx / (float)halfW;
            float ny = (float)ly / (float)halfH;
            if (nx * nx + ny * ny > 1.0f) continue;

            int sx = (int)((nx * 0.5f + 0.5f) * (FACE_TEX_SIZE - 1));
            int sy = (int)((ny * 0.5f + 0.5f) * (FACE_TEX_SIZE - 1));
            if (sx < 0) sx = 0;
            if (sx >= FACE_TEX_SIZE) sx = FACE_TEX_SIZE - 1;
            if (sy < 0) sy = 0;
            if (sy >= FACE_TEX_SIZE) sy = FACE_TEX_SIZE - 1;

            COLORREF c = (COLORREF)(buf[sy * FACE_TEX_SIZE + sx] & 0x00FFFFFF);
            Vec2 w = addVec(e.pos, rotateLocal(makeVec((float)lx, (float)ly), e.angle));
            putpixel((int)w.x, (int)w.y, c);
        }
    }

    setlinecolor(RGB(96, 82, 62));
    for (int i = -halfW; i <= halfW; i += 2) {
        float nx = (float)i / (float)halfW;
        float core = 1.0f - nx * nx;
        if (core < 0.0f) continue;
        int yy = (int)(sqrtf(core) * halfH);
        Vec2 a = addVec(e.pos, rotateLocal(makeVec((float)i, (float)(-yy)), e.angle));
        Vec2 b = addVec(e.pos, rotateLocal(makeVec((float)i, (float)yy), e.angle));
        if (i == -halfW || i == halfW) {
            putpixel((int)a.x, (int)a.y, RGB(96, 82, 62));
            putpixel((int)b.x, (int)b.y, RGB(96, 82, 62));
        }
    }
}
